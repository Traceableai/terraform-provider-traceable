...
func ResourceCustomSignatureAlertRule() *schema.Resource {
	return &schema.Resource{
		Create: ResourceCustomSignatureAlertCreate,
		Read:   ResourceCustomSignatureAlertRead,
		Update: ResourceCustomSignatureAlertUpdate,
		Delete: ResourceCustomSignatureAlertDelete,
...
func ResourceCustomSignatureAlertRead(d *schema.ResourceData, meta interface{}) error {
	id := d.Id()
	...
	if ruleEffect, ok := ruleDetails["ruleEffect"].(map[string]interface{}); ok {
		...
		if effects, ok := ruleEffect["effects"].([]interface{}); ok {
			for _, effect := range effects {
				...
				if effectMap["ruleEffectType"] == "AGENT_EFFECT" {
...
func ResourceCustomSignatureAlertUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	query := fmt.Sprintf(ALERT_UPDATE_QUERY, id, disabled, name, description, rule_type, finalAgentEffectQuery, alert_severity, finalReqResConditionsQuery, customSecRuleQuery, finalAttributeBasedConditionsQuery, envQuery)
